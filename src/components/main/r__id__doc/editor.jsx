...
const DocEditor = ({ state__str_selected_elem, fn_setter__close_editor }) => {
  // State(Local)
  ...
  useEffect(() => {
    if (state__obj_is_key_down.k_13 && state__obj_is_key_down.k_17) {
      ...
      fn_POST__add_doc_elem(state__str_selected_elem, state__str_txta_value);
  ...
  return (
    <article
      className={`comp_editor ${
        state__str_selected_elem ? "state__active" : undefined
      }`}
    >
      <form
      ...
      >
        <p>{`${state__str_selected_elem} Editor - Ctrl+Enter | Click Button`}</p>
        <textarea
          placeholder="Input..."
